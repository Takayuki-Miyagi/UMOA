...
SRCDIR_HF = submodule/HartreeFock/src
SRCDIR_Ite = submodule/Iteration/src
SRCDIR_UMOA = main
DEPDIR = .
OBJDIR = obj
...
SRCC_Ite:=$(wildcard $(SRCDIR_Ite)/*.c)
SRCF77_Ite:=$(wildcard $(SRCDIR_Ite)/*.f)
SRCF90_Ite:=$(wildcard $(SRCDIR_Ite)/*.f90)
SRCF95_Ite:=$(wildcard $(SRCDIR_Ite)/*.F90)
OBJC_Ite:=$(addprefix $(OBJDIR)/, $(patsubst %c, %o, $(notdir $(SRCC_Ite))))
OBJF77_Ite:=$(addprefix $(OBJDIR)/, $(patsubst %f, %o, $(notdir $(SRCF77_Ite))))
OBJF90_Ite:=$(addprefix $(OBJDIR)/, $(patsubst %f90, %o, $(notdir $(SRCF90_Ite))))
OBJF95_Ite:=$(addprefix $(OBJDIR)/, $(patsubst %F90, %o, $(notdir $(SRCF95_Ite))))
SRCS_Ite= $(SRCC_Ite) $(SRCF77_Ite) $(SRCF90_Ite) $(SRCF95_Ite)
OBJS_Ite= $(OBJC_Ite) $(OBJF77_Ite) $(OBJF90_Ite) $(OBJF95_Ite)

SRCC_UMOA:=$(wildcard $(SRCDIR_UMOA)/*.c)
SRCF77_UMOA:=$(wildcard $(SRCDIR_UMOA)/*.f)
SRCF90_UMOA:=$(wildcard $(SRCDIR_UMOA)/*.f90)
SRCF95_UMOA:=$(wildcard $(SRCDIR_UMOA)/*.F90)
OBJC_UMOA:=$(addprefix $(OBJDIR)/, $(patsubst %c, %o, $(notdir $(SRCC_UMOA))))
OBJF77_UMOA:=$(addprefix $(OBJDIR)/, $(patsubst %f, %o, $(notdir $(SRCF77_UMOA))))
OBJF90_UMOA:=$(addprefix $(OBJDIR)/, $(patsubst %f90, %o, $(notdir $(SRCF90_UMOA))))
OBJF95_UMOA:=$(addprefix $(OBJDIR)/, $(patsubst %F90, %o, $(notdir $(SRCF95_UMOA))))
SRCS_UMOA= $(SRCC_UMOA) $(SRCF77_UMOA) $(SRCF90_UMOA) $(SRCF95_UMOA)
OBJS_UMOA= $(OBJC_UMOA) $(OBJF77_UMOA) $(OBJF90_UMOA) $(OBJF95_UMOA)



SRCS_ALL = $(SRCS) $(SRCS_LinAlg) $(SRCS_HF) $(SRCS_Ite) $(SRCS_UMOA)
OBJS_ALL = $(OBJS) $(OBJS_LinAlg) $(OBJS_HF) $(OBJS_Ite) $(OBJS_UMOA)
...
$(OBJDIR)/%.o:$(SRCDIR_Ite)/%.c
	$(CC) $(CFLAGS) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_Ite)/%.f
	$(FC) $(FFLAGS) $(MODOUT) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_Ite)/%.f90
	$(FC) $(FFLAGS) $(DFLAGS) $(MODOUT) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_Ite)/%.F90
	$(FC) $(FFLAGS) $(DFLAGS) $(MODOUT) -o $@ -c $<

$(OBJDIR)/%.o:$(SRCDIR_UMOA)/%.c
	$(CC) $(CFLAGS) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_UMOA)/%.f
	$(FC) $(FFLAGS) $(MODOUT) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_UMOA)/%.f90
	$(FC) $(FFLAGS) $(DFLAGS) $(MODOUT) -o $@ -c $<
$(OBJDIR)/%.o:$(SRCDIR_UMOA)/%.F90
	$(FC) $(FFLAGS) $(DFLAGS) $(MODOUT) -o $@ -c $<
